...
__global__ void ReadKernel(T *a, T *b, size_t N, size_t num_chunks) {
  ...
  size_t HBM3_BLOCK_SIZE = (size_t)24 * 1024 * 1024 * 1024;
  size_t HBM3_BLOCK_ADDR = HBM3_BLOCK_SIZE / sizeof(VecType);
  int group_size = N / 8;
  int HBM3_IDX = idx / group_size;

  a = &a[HBM3_IDX * HBM3_BLOCK_ADDR];
...
//   CHECK_HIP_ERROR(hipMalloc(&d_data_a, num_elems * sizeof(VecType)));
  CHECK_HIP_ERROR(hipMalloc(&d_data_a, (size_t)178 * 1024 * 1024 * 1024));
